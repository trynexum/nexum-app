...
import { downloadGraphNodeRelease, extractGz, extractZipAndGetExe, getLatestGraphNodeRelease, moveFileToBinDir, } from '../command-helpers/local-node.js';
...
async function installGraphNode(tag, binDir) {
    ...
    let progressBar;
    const downloadPath = await downloadGraphNodeRelease(latestRelease, tmpDir, (downloaded, total) => {
        if (!total)
            return;
